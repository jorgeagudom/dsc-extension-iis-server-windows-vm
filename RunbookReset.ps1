...
$User = "Jorge.Agudo@tajamar365.com"
...
$WebServer = Invoke-WebRequest "http://104.209.241.171" -UseBasicParsing
...
if ($Estado -eq 200){
        ...
        Write-Verbose "Todo OK"
        exit 0
        }

else {

        $restart1 = Restart-AzureRmVM -ResourceGroupName "RG-MatWebServer" -Name "VMweb1"
        $restart2 = Restart-AzureRmVM -ResourceGroupName "RG-MatWebServer" -Name "VMweb2"

        if ($restart1.Status -and $restart2.Status -eq "Succeeded"){

            Write-Output "Las maquinas estan reiniciadas"
          ...
          {
            $i++
            $restart3 = Restart-AzureRmVM -ResourceGroupName "RG-MatWebServer" -Name "VMweb1"
            $restart4 = Restart-AzureRmVM -ResourceGroupName "RG-MatWebServer" -Name "VMweb2"

          }While (($restart3.Status -and $restart4.Status -ne "Succeeded") -or ($i -le 3))

          if ($restart3.Status -and $restart4.Status -ne "Succeeded"){

            Write-Output "ERROR de reinicio"
            ...
            Write-Output "Maquinas reiniciadas, pero algo pasa"
